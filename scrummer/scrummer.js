...
const getTitleDataConfiguration = () => ({
    story: {
...
const containsNodeWithScrummerClass = nodeList => {
    ...
    return Array.from(nodeList.values())
        .some(node => {
            ...
            return Array.from(node.classList.values())
                .some(
...
const calculatePointsForCard = card => {
    ...
    const titleDataConfiguration = getTitleDataConfiguration();
...
const calculatePointsForList = list => {
    ...
    titleDataConfiguration = getTitleDataConfiguration();
...
const checkForLists = () => {
    if (document.querySelectorAll('.list')
        .length > 0) {
